...
// Version 2.4 - Multi-API Support with Fallbacks + Image Support + Detailed Additives
// INCLUDES: Nutritionix Fix, Spoonacular API, Enhanced Scoring
...
class ProductService {
  ...
  static async fetchProductByBarcode(barcode) {
    ...
    // If not found and Spoonacular key exists, try Spoonacular
    if (!product && this.API_KEYS.SPOONACULAR) {
      try {
        console.log('📡 Trying Spoonacular...');
        product = await this.fetchFromSpoonacular(barcode);
        if (product) {
          source = 'Spoonacular';
          console.log('✅ Found in Spoonacular:', product.product_name);
        } else {
  ...
  /**
   * Spoonacular API - Good for grocery products and recipes
   */
  static async fetchFromSpoonacular(barcode) {
    ...
    try {
      const response = await fetch(
        `https://api.spoonacular.com/food/products/upc/${barcode}?apiKey=${this.API_KEYS.SPOONACULAR}`
      );
      ...
      if (data && data.id) {
        ...
        const nutritionResponse = await fetch(
          `https://api.spoonacular.com/food/products/${data.id}/nutritionWidget.json?apiKey=${this.API_KEYS.SPOONACULAR}`
        );
        
        const nutritionData = await nutritionResponse.json();
        console.log('Spoonacular nutrition:', nutritionData);
        
        // Convert Spoonacular format to Open Food Facts format
        const nutrients = {};
        
        // Process nutrients array
        if (nutritionData && nutritionData.nutrients) {
          nutritionData.nutrients.forEach(nutrient => {
            switch(nutrient.name) {
              case 'Sugar':
                nutrients['sugars_100g'] = (nutrient.amount / data.serving.number) * (100 / data.serving.size);
                break;
              case 'Sodium':
                // Convert mg to g per 100g
                nutrients['sodium_100g'] = (nutrient.amount / data.serving.number) * (100 / data.serving.size) / 1000;
                break;
              case 'Saturated Fat':
                nutrients['saturated-fat_100g'] = (nutrient.amount / data.serving.number) * (100 / data.serving.size);
                break;
              case 'Fiber':
                nutrients['fiber_100g'] = (nutrient.amount / data.serving.number) * (100 / data.serving.size);
                break;
              case 'Protein':
                nutrients['proteins_100g'] = (nutrient.amount / data.serving.number) * (100 / data.serving.size);
                break;
              case 'Calories':
                nutrients['energy-kcal_100g'] = (nutrient.amount / data.serving.number) * (100 / data.serving.size);
                break;
              case 'Carbohydrates':
                nutrients['carbohydrates_100g'] = (nutrient.amount / data.serving.number) * (100 / data.serving.size);
                break;
              case 'Fat':
                nutrients['fat_100g'] = (nutrient.amount / data.serving.number) * (100 / data.serving.size);
                break;
            }
          });
        }
        ...
        if (data.ingredientList) {
          ingredientsText = data.ingredientList;
        ...
        // Build categories
        const categories = [];
        if (data.aisle) categories.push(data.aisle);
        if (data.breadcrumbs) categories.push(...data.breadcrumbs);
        ...
        if (ingredientsText) {
          ...
          // Check for E-numbers
          const eNumberMatches = ingredientsLower.match(/\be\d{3}\b/g);
          if (eNumberMatches) {
            additivesTags.push(...eNumberMatches.map(e => `en:${e}`));
          }
        }
        
        // Determine NOVA group based on ingredients count and processing indicators
        let novaGroup = 1;
        ...
        return {
          ...
          image_front_url: data.image || data.images?.[0],
          additives_tags: additivesTags,
          nova_group: novaGroup,
          // Additional Spoonacular-specific data
          badges: data.badges || [],
          important_badges: data.importantBadges || [],
          serving_size: `${data.serving?.size}${data.serving?.unit}` || '100g'
        };
      }
